                ...
                @if ($tab === 'sell' && isset($items))
                    @forelse ($items as $item)
                        @php
                            ...
                            $isSold =
                                ($item->purchase && $item->purchase->exists()) || ($item->status ?? null) === 'sold';
                        @endphp
                        <li class="item-card">
                            <a class="item-card__link" href="{{ route('items.show', $item) }}">
                                <div class="item-card__image-wrap">
                                    ...
                                    @endif
                                    <img class="item-card__image" src="{{ $src }}" alt="{{ $item->title }}"
                                        width="290" height="290" loading="lazy"
                    ...
                    @forelse ($purchases as $purchase)
                        ...
                        <li class="item-card">
                            <a class="item-card__link" href="{{ $it ? route('items.show', $it) : '#' }}">
                                <div class="item-card__image-wrap">
                                    <span class="item-card__sold">Sold</span>
                                    <img class="item-card__image" src="{{ $src }}" alt="{{ $it->title ?? '商品' }}"
                                        width="290" height="290" loading="lazy"
                                        onerror="this.onerror=null; this.src='{{ asset('images/placeholder-290.png') }}';">
                                </div>
                                <div class="item-card__body">
