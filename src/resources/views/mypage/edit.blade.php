...
<div class="pfx">
    ...
    <form method="POST" action="{{ route('mypage.profile.update') }}" enctype="multipart/form-data" class="pfx__form">
        ...
        <div class="pfx__avatarRow">
            ...
            @if ($user->avatar_path)
            <img src="{{ Storage::url($user->avatar_path) }}?v={{ time() }}" alt="avatar" class="pfx__avatarImg">
            @else
        ...
        <div class="pfx__group">
            ...
            @error('name') <p class="pfx__error">{{ $message }}</p> @enderror
        </div>

        {{-- 郵便番号 --}}
        <div class="pfx__group">
            <label class="pfx__label">郵便番号</label>
            <input type="text" name="postal_code" value="{{ old('postal_code', $user->postal_code) }}" placeholder="123-4567" class="pfx__input">
            @error('postal_code') <p class="pfx__error">{{ $message }}</p> @enderror
        </div>
        ...
        <div class="pfx__group">
            ...
            @error('address') <p class="pfx__error">{{ $message }}</p> @enderror
        </div>
        ...
        <div class="pfx__group">
            ...
            @error('building') <p class="pfx__error">{{ $message }}</p> @enderror
        </div>
