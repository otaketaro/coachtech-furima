    ...
    <div class="pg-item-show__grid product-detail">
        ...
        <div class="item-visual product-image-area">
            <div class="item-visual__box">
                ...
                $src = $isUrl
                ? $p
                : ($p ? asset('storage/'.$p) : asset('images/placeholder-290.png'));
                @endphp

                <img
                    src="{{ $src }}"
                    alt="{{ $item->title }}"
                    class="item-visual__img"
            ...
            <div class="item-metrics">
                ...
                <div class="metric">
                    ...
                    <form action="{{ $isLiked ? route('likes.destroy', $item) : route('likes.store', $item) }}"
                        ...
                        <button type="submit"
                            class="metric-btn {{ $isLiked ? 'is-active' : '' }}"
                    ...
                    <a href="{{ route('login') }}" class="metric-btn is-ghost" aria-label="ログインしていいね">
                        <img class="metric__img" src="{{ asset('images/icon-star.png') }}" width="40" height="40" alt="">
                ...
                {{-- コメント数（表示のみ） --}}
                <div class="metric">
                    <img class="metric__img" src="{{ asset('images/icon-bubble.png') }}" width="40" height="40" alt="">
                    <span class="metric__num">{{ $item->comments_count ?? optional($item->comments)->count() ?? 0 }}</span>
                </div>
            </div>
                ...
                <ul class="comment-list">
                    ...
                    <li class="comment">
                        <div class="comment__avatar">
                            ...
                            <img src="{{ asset('storage/'.$comment->user->avatar_path) }}" alt="{{ $comment->user->name }}">
                            @else
                        ...
                        <div class="comment__body">
                            <div class="comment__meta">
                                ...
                                <span class="comment__time">{{ optional($comment->created_at)->format('Y/m/d H:i') }}</span>
                            </div>
