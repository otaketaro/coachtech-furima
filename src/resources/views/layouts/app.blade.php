    ...
    <header class="site-header">
        <div class="site-header__inner">
            ...
            @unless (request()->routeIs(['login', 'register']))
                {{-- 検索フォーム --}}
                <form method="GET" action="{{ route('items.index') }}" class="site-header__search" id="site-search-form"
                    role="search">
                    <input type="hidden" name="tab" value="{{ request('tab') }}">
                    <label class="sr-only" for="header-search">商品名で検索</label>
                    <input id="header-search" type="search" {{-- ネイティブの“×”が出る --}} name="q"
                        value="{{ request('q') }}" placeholder="なにをお探しですか？" aria-label="商品名で検索"
